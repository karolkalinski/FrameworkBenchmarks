...
3. Create `vertx-oracle.dockerfile`
...
You can create multiple implementations and they will all conform to `[name in benchmark_config.json].dockerfile`. For example, the `default` implementation in `benchmark_config.json` will be `vertx-oracle.dockerfile`, but if you wanted to make another implementation that did only the database tests for MySQL, you could make `vertx-oracle-mysql.dockerfile` and have an entry in your `benchmark_config.json` for `vertx-oracle-mysql`.

4. Test your application

        $ tfb --mode verify --test vertx-oracle
...
Edit `.travis.yml` to ensure that Travis-CI will automatically run our verification tests against your new test. This file is kept in alphabetical order, so find where `TESTDIR=Java/vertx-oracle` should be inserted under `env > matrix` and put it there.
...
# vertx-oracle Benchmarking Test
